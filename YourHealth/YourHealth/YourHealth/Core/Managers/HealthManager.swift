...
class HealthManager {
    ...
    func initializeHealthKitAccess() {
        Task {
            do {
                try await requestHealthKitAccess()
            } catch {
                print("HealthKit init error: \(error.localizedDescription)")
            }
        }
    }
    ...
    func fetchTodayBurnedCalories(completion: @escaping(Result<Double, Error>) -> Void) {
        let calories = HKQuantityType(.activeEnergyBurned)
